...
| Registration     | Self-registration is on by default and gives new accounts the Employee role immediately. Create the first Super Admin (above) **before** the site is reachable — until one exists, the first person to register becomes Super Admin. On a public URL, turn on "Require administrator approval" or turn registration off in Settings. |
