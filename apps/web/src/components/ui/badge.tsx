...
export function Badge({ tone = 'gray', children, className }: { tone?: Tone; children: React.ReactNode; className?: string }) {
  return (
    <span className={clsx('inline-flex items-center whitespace-nowrap rounded-md px-2 py-0.5 text-xs font-medium ring-1 ring-inset', tones[tone], className)}>
      {children}
    </span>
  );
}
...
const TONE_MAP: Partial<Record<LabelGroup, Record<string, Tone>>> = {
  ...
  maintenanceStatus: { SCHEDULED: 'blue', IN_PROGRESS: 'amber', COMPLETED: 'green', CANCELLED: 'gray' },
  priority: { LOW: 'gray', MEDIUM: 'blue', HIGH: 'amber', CRITICAL: 'red' },
  ticketStatus: { OPEN: 'blue', IN_PROGRESS: 'amber', ON_HOLD: 'violet', RESOLVED: 'green', CLOSED: 'gray' },
  auditStatus: { DRAFT: 'gray', IN_PROGRESS: 'amber', IN_REVIEW: 'blue', CLOSED: 'green', CANCELLED: 'gray' },
  auditResult: { PENDING: 'gray', FOUND: 'green', MISSING: 'red', UNEXPECTED: 'amber', WRONG_LOCATION: 'amber', DAMAGED: 'red' },
...
export function StatusBadge({ group, value }: { group: LabelGroup; value: string | null | undefined }) {
