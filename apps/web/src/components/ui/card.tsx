...
export function CardHeader({
  ...
  return (
    <div className={clsx('flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-slate-800', className)}>
    ...
    >
      {items.map((item) => (
        <div key={item.label} className="min-w-0">
          ...
          <dd className="mt-1 break-words text-sm text-slate-900 dark:text-slate-100">{item.value ?? '—'}</dd>
        </div>
      ))}
    </dl>
  );
...
export function StatCard({
  ...
  const content = (
    <Card className={clsx('p-4', href && 'transition-colors hover:border-blue-300 dark:hover:border-blue-800')}>
  ...
  return href ? (
    <a href={href} className="block rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
      {content}
    </a>
  ) : (
    content
