...
export class PdfService {
  ...
  render(build: (doc: Pdf) => void | Promise<void>, options: PDFKit.PDFDocumentOptions = {}): Promise<Buffer> {
  ...
  header(doc: Pdf, title: string, subtitle?: string): void {
    ...
    if (subtitle) doc.moveDown(0.2).font('Helvetica').fontSize(10).fillColor('#475569').text(subtitle);
    doc
      .moveDown(0.6)
      .strokeColor('#cbd5e1')
  ...
  section(doc: Pdf, title: string): void {
    ...
    doc.moveDown(0.4).font('Helvetica-Bold').fontSize(12).fillColor('#0f172a').text(title).moveDown(0.3);
  }
  ...
  keyValues(doc: Pdf, rows: KeyValueRow[]): void {
    ...
    for (const row of rows) {
      ...
      const y = doc.y;
      doc.font('Helvetica').fontSize(10).fillColor('#64748b').text(row.label, left, y, { width: labelWidth });
      doc
        .font('Helvetica')
        .fillColor('#0f172a')
  ...
  table(doc: Pdf, columns: { label: string; width: number }[], rows: (string | number | null | undefined)[][]): void {
    const left = doc.page.margins.left;
    ...
    const drawRow = (cells: (string | number | null | undefined)[], bold: boolean) => {
      doc.font(bold ? 'Helvetica-Bold' : 'Helvetica').fontSize(8.5).fillColor(bold ? '#0f172a' : '#1e293b');
      ...
      const y = doc.y;
      if (bold) doc.rect(left, y - 2, printable, height).fill('#f1f5f9').fillColor('#0f172a');
      let x = left;
      ...
      doc.y = y + height;
      doc.strokeColor('#e2e8f0').moveTo(left, doc.y - 2).lineTo(left + printable, doc.y - 2).stroke();
    };
