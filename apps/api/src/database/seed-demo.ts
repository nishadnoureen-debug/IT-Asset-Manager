...
async function main() {
  ...
  try {
    ...
    const loc = {
      HQ: hq,
      F1: await prisma.location.create({
        data: { code: 'HQ-F1', name: 'Head Office · Floor 1', type: 'FLOOR', parentId: hq.id },
      }),
      F2: await prisma.location.create({
        data: { code: 'HQ-F2', name: 'Head Office · Floor 2', type: 'FLOOR', parentId: hq.id },
      }),
      SRV: await prisma.location.create({
        data: { code: 'HQ-SRV', name: 'Server Room', type: 'DATA_CENTER', parentId: hq.id },
      }),
      WH: await prisma.location.create({
        data: { code: 'WH', name: 'IT Store Room', type: 'WAREHOUSE', city: 'Dubai' },
      }),
      REMOTE: await prisma.location.create({
        data: { code: 'REMOTE', name: 'Remote / Home office', type: 'REMOTE' },
      }),
    };
    ...
    await prisma.department.update({
      where: { id: dept.IT.id },
      data: { managerId: emp.E1001.id },
    });
    await prisma.department.update({
      where: { id: dept.FIN.id },
      data: { managerId: emp.E1003.id },
    });
    ...
    for (const [email, displayName, role, employeeId] of users) {
      const u = await prisma.user.create({
        data: {
          ...
          employeeId,
          roles: { create: { roleId: roles[role].id } },
        },
      });
      userIds[role] = u.id;
    }
    ...
    // ── Vendors & purchases ─────────────────────────────────────────────────
    const vendor = {
      dell: await prisma.vendor.create({
        data: {
          name: 'Dell Technologies',
          types: ['MANUFACTURER', 'SUPPLIER', 'WARRANTY_PROVIDER'],
          email: 'sales@dell.example',
          website: 'https://www.dell.com',
        },
      }),
      lenovo: await prisma.vendor.create({
        data: {
          name: 'Lenovo',
          types: ['MANUFACTURER', 'WARRANTY_PROVIDER'],
          website: 'https://www.lenovo.com',
        },
      }),
      apple: await prisma.vendor.create({
        data: { name: 'Apple', types: ['MANUFACTURER', 'SUPPLIER'] },
      }),
      cisco: await prisma.vendor.create({ data: { name: 'Cisco', types: ['MANUFACTURER'] } }),
      microsoft: await prisma.vendor.create({
        data: { name: 'Microsoft', types: ['SOFTWARE_PUBLISHER'] },
      }),
      adobe: await prisma.vendor.create({ data: { name: 'Adobe', types: ['SOFTWARE_PUBLISHER'] } }),
      service: await prisma.vendor.create({
        data: {
          name: 'Gulf IT Services LLC',
          types: ['SERVICE_PROVIDER', 'SUPPLIER'],
    ...
    const vendorIds: Record<string, string> = Object.fromEntries(
      Object.entries(vendor).map(([k, v]) => [k, v.id]),
    );
      ...
      const asset = await assets.create(
        {
          ...
          warrantyProviderId: ['dell', 'lenovo'].includes(a.vendor)
            ? vendorIds[a.vendor]
            : undefined,
    ...
    const assign = (
      ...
      assignments.assign(
        created[key],
        { employeeId: emp[employee].id, condition, accessories, notes: 'Standard issue', ...extra },
        admin,
      );

    await assign('L1', 'E1002', 'GOOD', [
      { accessoryId: charger.id, quantity: 1 },
      { accessoryId: mouse.id, quantity: 1 },
    ]);
    await assign('L2', 'E1004', 'GOOD', [
      { accessoryId: charger.id, quantity: 1 },
      { accessoryId: bag.id, quantity: 1 },
    ]);
    await assign(
      'L4',
      'E1007',
    ...
    await assign('L5', 'E1006', 'NEW', [
      { accessoryId: charger.id, quantity: 1 },
      { accessoryId: dock.id, quantity: 1 },
    ]);
    ...
    await assignments.assign(
      created.PJ1,
      { locationId: loc.F1.id, condition: 'NEW', notes: 'Meeting room 1.04' },
      admin,
    );
    await assignments.assign(
      created.S1,
      { locationId: loc.SRV.id, condition: 'NEW', notes: 'Rack A, U12-14' },
      admin,
    );

    // Return, then transfer — history shows the full chain.
    await assign('L3', 'E1005', 'GOOD', [{ accessoryId: charger.id, quantity: 1 }]);
    await assignments.returnAsset(
      created.L3,
      { condition: 'FAIR', returnTo: 'IN_STOCK', notes: 'Replaced by newer device' },
      admin,
    );
    ...
    await assignments.transfer(
      created.M2,
      {
        employeeId: emp.E1009.id,
        reason: 'Desk move to operations coordinator',
        condition: 'GOOD',
      },
      admin,
    );
    ...
    await prisma.maintenance.create({
      data: {
        ...
        status: 'SCHEDULED',
        priority: 'LOW',
        title: 'Quarterly printer service',
        technicianId: tech,
        vendorId: vendor.service.id,
    ...
    const repair = await prisma.maintenance.create({
      data: {
        ...
        description: 'Several keys intermittently fail.',
        technicianId: tech,
        vendorId: vendor.lenovo.id,
    ...
    await assets.retire(
      created.L8,
      { reason: 'End of life — battery swollen, out of warranty' },
      admin,
    );

    // ── Software & licences ─────────────────────────────────────────────────
    const m365 = await prisma.software.create({
      data: {
        name: 'Microsoft 365',
        version: 'E3',
        publisherId: vendor.microsoft.id,
        category: 'Productivity',
      },
    });
    const acrobat = await prisma.software.create({
      data: {
        name: 'Adobe Acrobat Pro',
        version: '2024',
        publisherId: vendor.adobe.id,
        category: 'Documents',
      },
    });
    const win = await prisma.software.create({
      data: {
        name: 'Windows 11 Pro',
        publisherId: vendor.microsoft.id,
    ...
    const m365Lic = await prisma.softwareLicense.create({
      data: {
        softwareId: m365.id,
        name: 'M365 E3 annual',
        licenseType: 'SUBSCRIPTION',
        seats: 50,
        vendorId: vendor.microsoft.id,
    ...
    const acroLic = await prisma.softwareLicense.create({
      data: {
        softwareId: acrobat.id,
        name: 'Acrobat Pro team',
        licenseType: 'SUBSCRIPTION',
        seats: 3,
        vendorId: vendor.adobe.id,
    ...
    const winLic = await prisma.softwareLicense.create({
      data: { softwareId: win.id, name: 'Windows OEM', licenseType: 'OEM', seats: null },
    });
    for (const e of [
      'E1001',
      'E1002',
      ...
      await prisma.softwareAssignment.create({
        data: { licenseId: m365Lic.id, employeeId: emp[e].id, assignedById: admin.id },
      });
    }
    for (const e of ['E1003', 'E1004', 'E1005']) {
      await prisma.softwareAssignment.create({
        data: { licenseId: acroLic.id, employeeId: emp[e].id, assignedById: admin.id },
      });
    }
    for (const key of ['L1', 'L2', 'L3', 'D1', 'D2']) {
      await prisma.softwareAssignment.create({
        data: { licenseId: winLic.id, assetId: created[key], assignedById: admin.id },
      });
    }
    ...
    await prisma.auditSession.create({
      data: {
        name: 'Store room spot check',
        locationId: loc.WH.id,
        createdById: admin.id,
        notes: 'Start this audit and scan the store room assets.',
      },
    });
    ...
    console.log('\nDemo data loaded.');
    console.log(
      `  ${assetSpecs.length} assets, ${people.length} employees, 5 departments, 6 locations, alerts: ${JSON.stringify(alerts)}`,
    );
    console.log('\nDemo accounts (local only — change or delete before sharing this environment):');
    for (const [email, name, role] of users)
      console.log(`  ${role.padEnd(20)} ${email.padEnd(26)} ${name}`);
